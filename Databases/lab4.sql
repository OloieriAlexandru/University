1. select Studenti.nume||' '||Studenti.prenume as Student, Note.valoare as Nota, Profesori.nume||' '|| Profesori.prenume as Profesor 
...
2. select s.nume||' '||s.prenume as Student, note.valoare as Nota, cursuri.titlu_curs as Curs from studenti s 
	join note on s.nr_matricol = note.nr_matricol 
	join cursuri on cursuri.id_curs = note.id_curs 
	where cursuri.titlu_curs = 'BD' and note.valoare = 10; 
3. select nume, prenume, titlu_curs from profesori
	join didactic on profesori.id_prof = didactic.id_prof
	join cursuri on didactic.id_curs = cursuri.id_curs;
4. select nume, prenume, titlu_curs from profesori
...
5. select c.titlu_curs 
	from cursuri c
	minus
	select c.titlu_curs 
	from cursuri c natural join didactic d;
5rau. select nume, prenume, titlu_curs from profesori
	join didactic on profesori.id_prof = didactic.id_prof
	right outer join cursuri on didactic.id_curs = cursuri.id_curs;
6a. select nume, prenume, titlu_curs from profesori
...
6b. select c.titlu_curs, nume, prenume 
	from profesori p 
...
7. select s1.nume, s2.nume, to_char(s1.data_nastere, 'Day') as "Zi a saptamanii" from studenti s1
	...
	order by 3, 1;                                            
8. select s1.nume||' '||s1.prenume||' a luat nota mai mare decat '||s2.nume||' '||s2.prenume||' la materia '||cursuri.titlu_curs from studenti s1
