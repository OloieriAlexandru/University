...
-- ex2
select nume, prenume, avg(n1.valoare) 
	from studenti s1 
...
-- ex3
select nume, prenume, an, grupa, avg(n1.valoare) 
	from studenti s1 
	natural join note n1 
...
-- ex5
select * from studenti s1 where 1 = (select count(*) from studenti s2 where s2.grupa = s1.grupa);
-- ex6
...
-- ex7
select nume, (select AVG(valoare) from note where nr_matricol = s.nr_matricol) as medie from studenti s;
-- ex8v1
select * from cursuri c1 where credite = (select max(credite) from cursuri c2 where c1.an = c2.an);
-- ex8v2
select * from cursuri where (an,credite) in (select an, max(credite) from cursuri group by an);	
