1. select nume||' '||prenume as student, bursa from studenti where bursa = (select max(bursa) from studenti);
2. select nume, prenume, an, grupa from studenti where (grupa, an) in (select grupa, an from studenti where lower(nume) = 'arhire') and lower(nume) != 'arhire';
3. select distinct nume, prenume, an, grupa, min(valoare) 
	from studenti 
	natural join note 
...
4. select nume, prenume, grupa, avg(valoare) 
	from studenti 
	natural join note 
...
8. select * from (select prenume from studenti order by 1 asc) where rownum <= 5 
	minus 
	select * from (select prenume from studenti order by 1 asc) where rownum <= 4;
8v2. select prenume from
...
9. select * from 
 ...
 minus
   select * from 
	...
	order by 7 desc)
	where rownum <= 2;     
10. select nume, prenume, max(valoare), titlu_curs 
	from studenti s 
	join note n on s.nr_matricol = n.nr_matricol 
	join cursuri c on n.id_curs = c.id_curs
