-- ex1
select s1.nume, s2.nume, abs(s1.data_nastere - s2.data_nastere) as diferenta_varsta_zile 
	from studenti s1 cross join studenti s2 
	where s1.nr_matricol < s2.nr_matricol 
	order by 3;
-- ex2
select 'Studentul '||s.nume||' '||s.prenume||' ar putea fi prieten cu profesorul '||trim(p.nume)||' '||trim(p.prenume) as Prietenii 
...
select distinct titlu_curs 
	...
	where n.valoare > 8;                        
-- ex3b
select c.titlu_curs, max(n.valoare) as nota_maxima 
	from cursuri c 
	join note n on c.id_curs = n.id_curs 
	group by titlu_curs 
...
select distinct s.nume||' '||s.prenume as student 
	...
	where n2.valoare < 7 ;
-- ex4b
select s.nume||' '||s.prenume as Student, min(n.valoare) as nota_minima 
	from studenti s 
	join note n on s.nr_matricol = n.nr_matricol 
	group by s.nume||' '||s.prenume 
	having min(n.valoare) >= 7;
-- ex5
select distinct s.nume||' '||s.prenume as Student, to_char(n.data_notare, 'Day') as Zi_din_saptamana, n.valoare as Nota 
	from studenti s 
	join note n on s.nr_matricol = n.nr_matricol 
...
-- ex6
select s.nume, s.prenume, n.valoare as Nota, trim(to_char(n.data_notare,'Month')) ||', '||trim(to_char(n.data_notare,'yyyy')) as Data, decode(to_char(last_day(n.data_notare),'dd'),'28','+','29','+',null) as Grea 
	from studenti s 
	natural join note n 
	where n.valoare >= 5;                                                                                             
